          ...
          <form>
            <div :class="{on:!isUsernameLogin}">
              <section class="login_message">
                <input type="tel" v-validate="'required|phone'" name="phone" v-model="phone" maxlength="11" placeholder="手机号">
                <span style="color: red;" v-show="errors.has('phone')">{{ errors.first('phone') }}</span>
                <button :disabled="!rightPhoneNum || count > 0" class="get_verification"
                  @click.prevent='getcode' 
                  :class="{rightPhone:rightPhoneNum}"
                >
                  {{count? `${count}后可以发送请求`:'获取验证码'}}
                  </button>
              </section>
            ...
            <div :class="{on:isUsernameLogin}">
              <section>
                <section class="login_message">
                  <input type="tel" maxlength="11" placeholder="手机/邮箱/用户名" v-validate="'required'" name="username" v-model="username">
                  <span style="color: red;" v-show="errors.has('username')">{{ errors.first('username') }}</span>
                </section>
                <section class="login_verification">
                  <input :type="isShowPwd? 'tel' : 'password'" v-validate="'required'" maxlength="8" placeholder="密码" name="pwd" >
                ...
                <section class="login_message">
                  <input v-model="captcha" name="captcha" type="text" maxlength="11" placeholder="验证码" v-validate="'required'">
                  <span style="color: red;" v-show="errors.has('captcha')">{{ errors.first('captcha') }}</span>
                  <img ref="getcha" @click="getchafn" class="get_verification" src="../../common/images/captcha.svg" alt="captcha">
                </section>
              </section>
            </div>
...
export default {
  ...
  methods:{
    getchafn(){
      this.$refs.getcha.src = 'http://localhost:4000/captcha?time=' + Date.now()
    },
    async getcode(){
      this.count = 5;
      let timer = setInterval(() => {
        this.count--;
        this.count === 0 && clearInterval(timer)
      },1000)
